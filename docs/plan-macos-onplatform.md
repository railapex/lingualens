...
- **Windows Store (Microsoft Store)**: Possible via Desktop Bridge (Win32 listing) — runs outside App Container sandbox, so all features work unchanged. Full App Container sandboxing would break text capture (UI Automation + SendInput are blocked cross-process). Desktop Bridge is how VS Code/Discord ship on the Store. Path: Partner Center account ($19), MSIX packaging (wrap existing NSIS output via MSIX Packaging Tool or `makeappx.exe`), `AppxManifest.xml` with identity/capabilities/visual assets, disable Tauri auto-updater for Store builds (Store handles updates). Certification notes: 2.9GB first-launch model download needs clear UX, must handle offline gracefully, Store tiles/screenshots in specific sizes required.
